...
public class DefaultAudioSource extends AbstractAudioSource implements Runnable {
    ...
    private IAudioDataProvider createRecorderNoLock(){
        ...
        SpeechLogUtil.log(String.format("DefaultAudioSource # createRecorderNoLock(), tmp.isInitialized():%b", tmp.isInitialized()));
        if(tmp.isInitialized()){
            try {
                tmp.start();
                SpeechLogUtil.log(String.format("DefaultAudioSource # createRecorderNoLock(), IAudioDataProvider tmp.start(), pos1)"));
            }catch (Exception e){
                e.printStackTrace();
                tmp.release();
                tmp = null;}
    ...
    public void run(){
        ...
        try {
            ...
            fireOnBegin();
            dataProvider.start();
            SpeechLogUtil.log(String.format("DefaultAudioSource # createRecorderNoLock(), IAudioDataProvider tmp.start(), pos2)"));
            ...
            while (true){
                try {
                    ...
                    if (readFrameCnt <= 0){
                        ...
                        onAudioError(ErrorIndex.ERROR_AUDIO_READ_FAILED, "audioRecord.read()<=0,readFrameCnt:" + readFrameCnt);
                        break;
                    }
                    ...
                    int status = pollStatusLocked();//获取状态
                    final boolean pausedOrStopped = (status == STATUS_PAUSED || status == STATUS_DEAD);
                    if (pausedOrStopped){
                        dataProvider.stop();//停止录音，AudioRecorder.stop()
                    }
                    //report progress
            ...
            //report finish
            fireOnEnd(errorCode, exception, totalFrameCnt);//结束录音回调
        }
        finally {
